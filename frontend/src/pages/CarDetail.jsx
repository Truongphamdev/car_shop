    ...
    return (
        <div className="container mx-auto py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 ms-8">
                <div>
                    <img id="mainImage" src={`http://localhost:8000/${data.car.car_image[0].image_url}`} className="w-full h-[400px] object-cover rounded-lg" />
                    <div className="grid grid-cols-4 gap-2 mt-4">
                        {data.car.car_image.map((image, index) => (
                            <img key={index} src={`http://localhost:8000/${image.image_url}`} onClick={() => document.getElementById("mainImage").src = `http://localhost:8000/${image.image_url}`} className="w-full h-20 object-cover rounded-lg cursor-pointer hover:opacity-80" />
                        ))}
                    </div>
                </div>
                <div>
            ...
            <button className="mt-6 px-6 py-3 bg-blue-500 text-white rounded-lg hover:bg-blue-600">Mua xe</button>
